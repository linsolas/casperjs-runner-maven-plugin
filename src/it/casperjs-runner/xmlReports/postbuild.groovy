...
file = new File(basedir, 'target/casperjs/TEST-test_coffee.xml');
assert file.exists();
assert file.text.contains('Fake coffee test');

file = new File(basedir, 'target/casperjs/TEST-test_js.xml');
assert file.exists();
assert file.text.contains('Fake js test');

file = new File(basedir, 'target/casperjs/TEST-subdir_test_coffee.xml');
assert file.exists();
assert file.text.contains('Subdir fake coffee test');

file = new File(basedir, 'target/casperjs/TEST-subdir_test_js.xml');
assert file.exists();
